...
from tfx.components.example_gen.base_example_gen_executor import BaseExampleGenExecutor  # noqa: E501
...
class _ParsedCsvToTfExample(beam.DoFn):
    ...
    def _make_column_handlers(self, column_infos: List[csv_decoder.ColumnInfo]):  # noqa: E501
        result = []
    ...
    def process(
            ...
            column_infos: List[csv_decoder.ColumnInfo]) -> Iterable[tf.train.Example]:  # noqa: E501
    ...
    def process(self, csv_filepath: str) -> Iterable[str]:
        with beam.io.filesystems.FileSystems.open(csv_filepath) as file:
            ...
            if not buffer.is_empty():
                raise ValueError(
                    'Csv record had unbalanced quotes. File: {}'.format(csv_filepath))  # noqa: E501
...
class _CsvToExample(beam.PTransform):
    ...
    def expand(
        ...
        logging.info(
            'Processing input csv data %s to TFExample.', self._csv_pattern)
