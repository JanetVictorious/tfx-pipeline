...
def _build_estimator(config, hidden_units=None, warm_start_from=None):
    ...
    :param config:
        tf.estimator.RunConfig defining the runtime environment for the
        estimator (including model_dir).
    :param int hidden_units:
        The layer sizes of the DNN (input layer first)
    :param warm_start_from:
        Optional directory to warm start from.
    :return:
...
def _eval_input_receiver_fn(tf_transform_output, schema):
    ...
    :return:
        EvalInputReceiver function, which contains:
            - Tensorflow graph which parses raw untransformed features,
              applies the tf-transform preprocessing operators.
